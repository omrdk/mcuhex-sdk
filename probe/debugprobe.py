...
class DebugProbe(object):
    ...
    def get_target_info(self) -> dict | None:
        """Return target MCU info after connect, or None if unavailable."""
        return None
    ...
    async def read_ascii(self, addr, nb):
    ...
    async def write_u08(self, addr, val):
        return await self.write_u16(addr, val)

    async def write_i08(self, addr, val):
